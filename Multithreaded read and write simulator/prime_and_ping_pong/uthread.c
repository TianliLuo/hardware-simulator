...
void initqueue (uthread_queue_t* queue) {
  queue->head = 0;
  queue->tail = 0;
}
...
// IA32-32
                ...
                "r" (to_thread),
                "r" (from_thread_state),
                "i" (offsetof (struct uthread_TCB, state)),
                "i" (offsetof (struct uthread_TCB, saved_sp)));
  ...
  } else if (from_thread->state == TS_RUNABLE)
    ready_queue_enqueue (from_thread);
